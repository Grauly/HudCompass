...
public class HUDCompassRenderer {
    ...
    public static void onRender(GuiGraphicsExtractor context, DeltaTracker renderTickCounter) {
        ...
        context.centeredText(textRenderer, "N", (int) ((width / 2f) + MathHelper.determineXPosOnCompass(angle, 180)), 5, -1);
        context.centeredText(textRenderer, "E", (int) ((width / 2f) + MathHelper.determineXPosOnCompass(angle, 90)), 5, -1);
        context.centeredText(textRenderer, "S", (int) ((width / 2f) + MathHelper.determineXPosOnCompass(angle, 0)), 5, -1);
        context.centeredText(textRenderer, "W", (int) ((width / 2f) + MathHelper.determineXPosOnCompass(angle, 270)), 5, -1);
    ...
    private static void drawWaypoints(GuiGraphicsExtractor context, Font textRenderer, int width, int playerAngle, Collection<Waypoint> waypoints, String currentDimension) {
        ...
        waypoints.forEach(w -> {
            if (!(w.isHidden()) && w.getDimensionID().equals(currentDimension)) {
                ...
                context.centeredText(textRenderer, w.getName(), ((width / 2) + pos) * 2, (13 - mc.font.lineHeight / 2) * 2, -1);
